...
function AutoConfig.process_auto_whitelist(src, reason, webhook, time)
    if not hexguard.AutoConfig then
    ...
    if detectedResource and detectedResource == "hexguard" then
    ...
    if isEvent and detectedResource then
        ...
        local newConfig, success = AutoConfig.append_to_table(configFile, "hexguard.EventWhitelist", isEvent)
        if success then
    ...
    elseif isUnregisteredEvent then
        ...
        local newConfig, success = AutoConfig.append_to_table(configFile, "hexguard.EventWhitelist", isUnregisteredEvent)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)
    ...
    elseif isSuspiciousEntity then
        ...
        local newConfig, success = AutoConfig.add_entity_resource(configFile, "hexguard.EntitySecurity", isSuspiciousEntity)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)
    ...
    elseif detectedResource then
        ...
        local newConfig, success = AutoConfig.add_entity_resource(configFile, "hexguard.EntitySecurity", detectedResource)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)

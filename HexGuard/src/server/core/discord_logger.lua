...
function DiscordLogger.initialize(config)
    ...
    if DiscordLogger.enabled then
        DiscordLogger.log_system(
            "System Started", 
            "The hexguard Anti-Cheat system has been initialized.",
            {
                {name = "Status", value = "✅ Online", inline = true},
                {name = "Version", value = "hexguard v1.0", inline = true}
            }
        )
    end
...
function DiscordLogger.send(webhook_type, title, description, fields, image_url, footer_text, thumbnail_url)
    ...
    local embed = {
        ...
        fields = fields or {},
        footer = {
            text = footer_text or "hexguard Anti-Cheat • " .. formattedDateTime
        }
    }
...
function DiscordLogger.log_ban(player_id, reason, ban_data, screenshot)
    ...
    TriggerClientEvent("hexguard:ForceSocialClubUpdate", player_id)
end

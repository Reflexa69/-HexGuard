...
function AdminWhitelist.initialize()
    ...
    if not _G.hexguard then _G.hexguard = {} end
    if not _G.hexguard.Whitelisted then _G.hexguard.Whitelisted = {} end
    ...
    RegisterNetEvent("hexguard:CheckWhitelist", function()
        local src = source
        local isWhitelisted = AdminWhitelist.isWhitelisted(src)
        TriggerClientEvent("hexguard:WhitelistResponse", src, isWhitelisted)
    end)
    
    RegisterNetEvent("hexguard:RequestAdminList", function()
        local src = source
        local adminList = {}
        
        if _G.hexguard and _G.hexguard.Whitelisted then
            for _, adminId in ipairs(_G.hexguard.Whitelisted) do
        ...
        TriggerClientEvent("hexguard:ReceiveAdminList", src, adminList)
    ...
    AddEventHandler("playerDropped", function()
        ...
        if _G.hexguard and _G.hexguard.Whitelisted then
            for i = #_G.hexguard.Whitelisted, 1, -1 do
                if tonumber(_G.hexguard.Whitelisted[i]) == tonumber(src) then
                    table.remove(_G.hexguard.Whitelisted, i)
                    break
                end
            end
        end
...
function AdminWhitelist.isAdmin(source)
    ...
    if hexguard.Permissions.Enabled then
        if AdminWhitelist.hasAcePermission(source, hexguard.Permissions.DefaultAce) then
            isAdmin = true
        else
            for group, ace in pairs(hexguard.Permissions.GroupAces) do
...
function AdminWhitelist.getManualAdmin(source)
    if not _G.hexguard or not _G.hexguard.Admins then
        return false
    end
    ...
    for _, identifier in pairs(identifiers) do
        for _, admin in pairs(_G.hexguard.Admins) do
...
function AdminWhitelist.isWhitelisted(source)
    ...
    if _G.hexguard and _G.hexguard.Whitelisted then
        local src = tonumber(source)
        for _, id in ipairs(_G.hexguard.Whitelisted) do
    ...
    if AdminWhitelist.isAdmin(source) then
        if _G.hexguard and _G.hexguard.Whitelisted then
            ...
            if not alreadyWhitelisted then
                table.insert(_G.hexguard.Whitelisted, src)

...
/**
 * Check if manifest has old hexguard include
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the old hexguard include
 */
function hasOldhexguard(manifestContent) {
    return manifestContent.includes(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`);
}

/**
 * Check if manifest has new hexguard includes
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the new hexguard includes
 */
function hasNewhexguard(manifestContent) {
...
function hasKeyFile(manifestContent) {
    return manifestContent.includes(`file "@${CURRENT_RESOURCE_NAME}/hexguard.key"`);
}

/**
 * Add hexguard includes to manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function addhexguard(manifestContent) {
    if (manifestContent.includes(`name "${CURRENT_RESOURCE_NAME}"`) || 
        manifestContent.includes(`name '${CURRENT_RESOURCE_NAME}'`)) {
        return manifestContent;
    }
    
    if (hasNewhexguard(manifestContent) && hasKeyFile(manifestContent)) {
        return manifestContent;
    }
    
    if (hasOldhexguard(manifestContent)) {
    ...
    if (!hasNewhexguard(manifestContent)) {
    ...
    if (!hasKeyFile(manifestContent)) {
        insertion += `\nfile "@${CURRENT_RESOURCE_NAME}/hexguard.key"`;
    }
...
/**
 * Remove hexguard references from manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content with hexguard removed
 */
function removehexguard(manifestContent) {
    manifestContent = manifestContent.replace(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`, '');
    ...
    manifestContent = manifestContent.replace(`file "@${CURRENT_RESOURCE_NAME}/hexguard.key"`, '');
...
/**
 * Update hexguard references in manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function updatehexguard(manifestContent) {
    if (hasNewhexguard(manifestContent) || !hasOldhexguard(manifestContent)) {
        return manifestContent;
    }
...
function checkAndFixDefaultKey() {
    const keyPath = path.join(RESOURCES_PATH, 'hexguard.key');
    ...
    try {
        if (fs.existsSync(keyPath)) {
            ...
            if (isDefaultKey) {
                Logger.critical(`Regenerating hexguard.key file due to default key...`);
                ...
                if (verifyKey === newKey) {
                    ...
                    Logger.info(`New key will be used by all hexguard components.`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hexguard.key file with secure random key.`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0); 
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensure hexguard.key exists with a secure random key
...
function ensureKeyFileExists(forceNew = false) {
    ...
    try {
        if (forceNew) {
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hexguard.key with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
        ...
        if (fs.existsSync(keyPath)) {
            ...
            if (!isDefaultKey) {
                Logger.info(`hexguard.key exists with custom key: ${keyPath}`);
                return true;
            }
            
            Logger.critical(`Default/invalid key detected in hexguard.key. Generating new secure key...`);
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Updated hexguard.key with new secure random key: ${keyPath}`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hexguard.key file with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensures hexguard starts before other resources in server.cfg
 * @return {boolean} Success status
 */
function ensurehexguardStartsFirst() {
    ...
    if (!serverCfgPath) {
        Logger.error('Could not find server.cfg. Please manually ensure hexguard starts first.');
        return false;
    }
...
/**
 * Install hexguard protection on all resources
 * @return {void}
 */
function installhexguard() {
    Logger.info(`Starting ${CURRENT_RESOURCE_NAME} installation...`);
    
    if (!ensureKeyFileExists()) {
        Logger.critical(`Failed to ensure hexguard.key exists. Installation may be incomplete.`);
    ...
    for (const resource of resources) {
        ...
        if (hasNewhexguard(content) && hasKeyFile(content)) {
            // Logger.debug(`${resource.name} already has ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = addhexguard(content);
    ...
    ensurehexguardStartsFirst();
    
    Logger.info(`${CURRENT_RESOURCE_NAME} installation complete. Success: ${successCount}, Failed: ${failCount}`);
}

/**
 * Uninstall hexguard protection from all resources
 * @return {void}
 */
function uninstallhexguard() {
    ...
    for (const resource of resources) {
        ...
        if (!hasOldhexguard(content) && !hasNewhexguard(content) && !hasKeyFile(content)) {
            Logger.debug(`${resource.name} doesn't have ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = removehexguard(content);
...
/**
 * Update hexguard protection on all resources
 * @return {void}
 */
function updatehexguard() {
...
/**
 * Get the encryption key from hexguard.key
 * @return {string} The encryption key to use
 */
function getEncryptionKey() {
    try {
        const keyFile = LoadResourceFile("hexguard", "hexguard.key");
        
        if (!keyFile || keyFile === "") {
            Logger.warning("hexguard key not found, using temporary key");
        ...
        if (keyFile === "Please Change this if u wont the ac just wont start until u do this delete this message and instead write a random combination of letters like a random password without any spaces" || 
            ...
            Logger.critical("Default hexguard key detected. Running ensureKeyFileExists() to create a new one.");
            checkAndFixDefaultKey();
            
            const updatedKey = LoadResourceFile("hexguard", "hexguard.key");
        ...
        Logger.error(`Failed to load hexguard encryption key: ${error.message}`);
...
RegisterCommand('securenewkey', (source, args, raw) => {
    ...
    Logger.info('Forcing regeneration of hexguard.key...');
    const success = ensureKeyFileExists(true);
    if (success) {
        Logger.success('Generated new hexguard encryption key. The server will restart to apply changes.');
    } else {
...
if (!fs.existsSync(path.join(RESOURCES_PATH, 'hexguard.key'))) {
    const keySuccess = ensureKeyFileExists();
    if (!keySuccess) {
        Logger.critical(`Could not create or verify hexguard.key`);

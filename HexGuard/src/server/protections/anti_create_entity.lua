...
function AntiCreateEntity.initialize()
    if hexguard and hexguard.Module and hexguard.Module.Entity and hexguard.Module.Entity.SecurityWhitelist then
        for _, entry in ipairs(hexguard.Module.Entity.SecurityWhitelist) do
            AntiCreateEntity.resourceWhitelist[entry.resource] = entry.whitelist
        end
        logger.info("Loaded " .. #hexguard.Module.Entity.SecurityWhitelist .. " whitelisted resources for entity security")
    end

    RegisterNetEvent("hexguard:Server:Methods:ModulePunish", function(screenshot, reason, webhook, time)
        local src = source
        if not src or src <= 0 then return end
        
        logger.warn(string.format("[hexguard] Entity Security: Player %s (%s) %s", 
        ...
        print("[hexguard] Incase this is a false ban go to config.lua and in it search hexguard.Module incase this is an event ban go to Events and then to whitelist and add the event")
        print("[hexguard] Incase its an entity ban go to Entity and SecurityWhitelist and add the resource name")
        if not screenshot and hexguard.Module.Entity.TakeScreenshot then
    ...
    RegisterNetEvent("hexguard:Server:Methods:Entity:CreateServer", function(entityId, resourceName, modelHash)
        local src = source
        AntiCreateEntity.allowHash(modelHash)
    end)

    RegisterNetEvent("hexguard:Server:Methods:Entity:Create", function(entityId, resourceName, modelHash)
        local src = source
    ...
    AddEventHandler('entityCreated', function(entity)
        ...
        if (population == 7 or population == 0) and owner ~= 0 and owner ~= -1 then
            ...
            elseif owner and modelHash then
                TriggerClientEvent("hexguard:CheckEntityResource", owner, NetworkGetNetworkIdFromEntity(entity), modelHash)
            end
        end
    end)

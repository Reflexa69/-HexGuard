...
function AntiExplosions.initialize()
    if not hexguard.Module.Explosions.ModuleEnabled then
        return
    end
    ...
    RegisterNetEvent("hexguard:Explosions:Whitelist", function(data)
    ...
    AddEventHandler('explosionEvent', function(sender, ev)
        ...
        if GetPlayerPing(sender) > 0 and hexguard.ExplosionsModule then
            if whitelist[sender] or hexguard.ExplosionsWhitelist[resourceName] then
        ...
        for k, v in pairs(hexguard.Protection.BlacklistedExplosions) do
            if ev.explosionType == v.id then
                ...
                if hexguard.Protection.CancelOtherExplosions then
                    for k, v in pairs(hexguard.Protection.BlacklistedExplosions) do
                        if ev.explosionType ~= v.id then
                            CancelEvent()
                        end

...
function AntiExecution.initialize()
    RegisterNetEvent("hexguard:Server_Callbacks:Detections:RegisterKnownMenus", function(menus)
    ...
    AddEventHandler("playerConnecting", function(_, _, deferrals)
        ...
        if GetPlayerEndpoint(source) == nil then
            deferrals.done("hexguard: Invalid connection detected.")
            CancelEvent()
        else
            deferrals.update("hexguard: Checking connection...")
            deferrals.done()
        end
    end)
end

...
local function setupErrorHandler()
    hexguardErrorHandler = function(err)
    ...
    AddEventHandler('onServerResourceStart', function(resource)
        if resource == GetCurrentResourceName() then
            ...
            error = function(err, level)
                if hexguardErrorHandler then
                    hexguardErrorHandler(err)
                end
...
local function registerServerCommands()
    ...
    RegisterCommand("securebanlist", function(source, args, rawCommand)
        ...
        print("===== hexguard Ban List (" .. #bans .. " most recent) =====")
    ...
    RegisterCommand("securehelp", function(source, args, rawCommand)
        ...
        print("===== hexguard Anti-Cheat Commands =====")
    ...
    RegisterCommand("securestats", function(source, args, rawCommand)
        ...
        print("===== hexguard System Statistics =====")
...
local function main()
    ...
    print("^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║                  ^2hexguard AntiCheat v1.2.1 Initializing^8               ║^7")
    print("^8╚══════════════════════════════════════════════════════════════════════════╝^7")
    ...
    logger.initialize(hexguard)
    logger.initialize({
        LogLevel = hexguard.LogLevel,
        UseWebhook = hexguard.UseWebhook,
        LogWebhook = hexguard.LogWebhook,
        MaxLogHistory = hexguard.MaxLogHistory,
        Debug = hexguard.Debug
    })
    print("^2│ ^2✓^7 Logger^7 initialized")

    print("^2│ ^5⏳^7 Discord Logger^7")
    discord_logger.initialize(hexguard)
    print("^2│ ^2✓^7 Discord Logger^7 initialized")
    print("^2│ ^5⏳^7 Debug Module^7")
    debug_module.initialize(hexguard)
    ...
    AddEventHandler("onResourceStop", function(resource_name)
        if resource_name == GetCurrentResourceName() then
            logger.info("hexguard AntiCheat is stopping...")
            logger.warn("hexguard AntiCheat is stopping...")
        end
    end)
    ...
    AddEventHandler("eventTriggered", function(event_name, source, ...)
        if hexguard.AutoConfig and auto_config and auto_config.process_auto_whitelist then
            auto_config.process_auto_whitelist(source, "Event triggered: " .. event_name, nil, nil)
        end

        if event_name and hexguard.SafeEvents then
            local safe_events = hexguard.SafeEvents
    ...
    AddEventHandler("resourceStarted", function(resource_name)
        ...
        if resource_to_check and hexguard.SafeResources then
            local safe_resources = hexguard.SafeResources
    ...
    RegisterNetEvent("hexguard:ClientLog", function(level, message)
        local source = source
        local player_name = GetPlayerName(source) or "Unknown"
    ...
    initialized = true
    print("\n^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║              ^2hexguard AntiCheat v1.2.1 Loaded Successfully^8            ║^7")
    ...
    discord_logger.log_system(
        "AntiCheat Started",
        "hexguard AntiCheat v1.2.1 has been successfully initialized.",
        {
    ...
    logger.info("hexguard AntiCheat v1.2.1 initialized successfully")
end
...
exports("module_punish", function(source, reason, webhook, time)
    ...
    if resource_to_check then
        if resource_to_check == GetCurrentResourceName() then
            logger.debug("Detection from hexguard itself, ignoring")
            return true
        end
...
RegisterNetEvent("hexguard:KickBannedPlayer", function(target)
...
RegisterNetEvent("hexguard:DisconnectMe", function()
    local source = source
...
RegisterNetEvent("hexguard:Server:Methods:PunishPlayer", function(id, reason, webhook, time)
    local source = source

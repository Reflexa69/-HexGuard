...
function Callbacks.initialize(isServer)
    if isServer then
        RegisterNetEvent("hexguard:callback:serverRequest", function(eventName, callbackId, ...)
            local src = source
            
            if Callbacks.callbackRegistry[eventName] then
                local result = Callbacks.callbackRegistry[eventName](src, ...)
                TriggerClientEvent("hexguard:callback:clientResponse", src, callbackId, result)
            end
        end)
    else
        RegisterNetEvent("hexguard:callback:clientResponse", function(callbackId, ...)
...
function Callbacks.triggerServerCallback(name, cb, ...)
    ...
    TriggerServerEvent("hexguard:callback:serverRequest", name, Callbacks.currentCallbackId, ...)
end
